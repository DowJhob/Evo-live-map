...
inline void set_const(float var)
{
...
inline bool IsDelimeter(QChar c)//Метод возвращает true, если проверяемый символ - разделитель ("пробел" или "равно")
{QString a = " =";
    if ((a.indexOf(c) != -1))
        return true;
    return false;
}
inline bool IsOperator(QChar c)//Метод возвращает true, если проверяемый символ - оператор
{
    QString a = "+-/*^()";
...
inline bool IsVar(QChar c)//Метод возвращает true, если проверяемый символ - переменная
{
    QString a = "x";
...
inline int GetPriority(QChar s)//Метод возвращает приоритет оператора
{
    QString a = "()+-*/^";
    switch (a.indexOf(s))
    {
...
inline QString get_notation_convert(QString input)
{
    QChar s;
    QString swst = "+-*/^";
...
inline float Computing(QString input, float variable_value)
{
...
inline float fast_calc(fast_calc_struct temp_fs, float x)
{
...
inline int int_fast_calc(fast_calc_struct int_temp_fs, int x)
{
...
inline qint64 typed(QString storagetype, uchar *in_buf, uint c, bool big)//кастуем данные к определенному типу
{
    if ( storagetype == "int8")
    {
