...
public:
    ...
    bool IsDelimeter(QChar c)//Метод возвращает true, если проверяемый символ - разделитель ("пробел" или "равно")
    {QString a = " =";
        if ((a.indexOf(c) != -1))
            return true;
        return false;
    }
    bool IsOperator(QChar c)//Метод возвращает true, если проверяемый символ - оператор
    {
        QString a = "+-/*^()";
    ...
    QString get_notation_convert(QString input)
    {
        QChar s;
        QString swst = "+-*/^";
        char counter = 0;
    ...
    fast_calc_struct* set_notation(QString input)                 //тут в отличии от get notation заполняем структуру с числами что бы не парсить каждый раз строку( 5 раз выигрыш)
    {
        init();
        QChar s;
        QString swst = "+-*/^";
    ...
    float fast_calc(fast_calc_struct temp_fs, float x)
    {
