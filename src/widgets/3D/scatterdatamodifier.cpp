...
void ScatterDataModifier::addData()
{
    ...
    {
            float c = fuelMap[i][j];
            float perc = c / 5; // get 10% mul 2
            float f = c + (float)(QRandomGenerator::global()->bounded(perc)) / 2.0f - (float)(QRandomGenerator::global()->bounded(perc)) / 2.0f;


        ptrToDataArray->setPosition(QVector3D(i,
                                              f,
                                              j));
        ptrToDataArray++;
    }
