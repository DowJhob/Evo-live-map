...
//const float sampleMin = 0.0f;
//const float sampleMax = 10000.0f;
    ...
    : m_graph(surface)
{
    ...
    //! [2]
    QImage heightMapImage(":/ico/mountain");
    ...
    m_graph->axisX()->setRange(0.0f, 25.0f);
    m_graph->axisY()->setRange(0.0f, 15.0f);
...
;
            ...
            (*newRow)[index++].setPosition(QVector3D(j, fuelMap[i][j], i
                                                     )
                                           );
        }
        *dataArray << newRow;
...
//        m_graph->axisX()->setRange(sampleMin, 25.0f);
//        m_graph->axisY()->setRange(0.0f, 15.0f);
