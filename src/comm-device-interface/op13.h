...
public:
    ...
    OP13( QString dllName = nullptr) : comm_device_interface( dllName)
    {
    }
    ~OP13()
    {
    ...
    {
//        _ftdi->FT_Write(_ftdi->ftHandle, msg.data(), msg.size(), &Reads );
//        qDebug() << "Writed bytes " << Reads;
//        _ftdi->FT_Read(_ftdi->ftHandle, in_buf, msg.size(), &Reads);    //читаем эхо
    }
